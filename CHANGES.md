## Version 9.7.9 (2026090300)
* New feature: Hidden Moodle courses can be connected to a booking option. The course selector of the option form lists every course the user may enrol into manually, hidden ones included (they are marked as "Hidden course"), so an option can be prepared and booked before the course is released - the booked users are enrolled as usual. The links to the connected course ("Start" on the booked button, "Go to Moodle course", the buttons of the Moodle app, the redirect to the course after login, the link on the enrol link page and the iCal location) are only shown while the course is visible, or to users who may see hidden courses (moodle/course:viewhiddencourses).
* Bugfix: The course selector of the option form no longer throws a coding exception for users who may not enrol into any course; they get the "No course selected" entry only.

## Version 9.7.9 (2026090200)
* New feature: New site setting "Course login required for the booked users report" (reportrequirecourselogin, enabled by default). When disabled, a site login is enough to open report.php and the Bookings Tracker (report2.php), e.g. for external trainers who are not enrolled in the course. The capability checks of the reports still apply.
...
* Bugfix: With multiple bookings enabled, a re-booking that landed in the same second as the previous booking (slow servers, CI runners) was stored as PREVIOUSLYBOOKED itself while the old answer stayed BOOKED; the next booking then updated that historic row in place instead of inserting a new answer, so one booking was silently lost (behat scenario booking_multiple_bookings.feature:29 failed - and kept failing in the automatic reruns - on slow CI cells). A live booking is now always the newest one, a previously booked answer is never reused, and an active answer is never shadowed by a historic one in booking_answers::get_users(). (Wunderbyte-GmbH/moodle-mod_booking#1550)
