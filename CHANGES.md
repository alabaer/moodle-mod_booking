## Version 9.7.9 (2026083105)
* Bugfix: Every upgrade step of the waiting list refactoring is anchored above the base version now (2026083101 - 2026083104). This branch sits on the USI line, which released 2026082801 and now 2026083100 itself, so the previous anchors (2026081700 - 2026082401) were below that base: a site coming from there skipped the creation of booking_waitlist_offers and booking_waitlist_declines, the recycling fields, the legacy chain migration and the waitlistopenmode column, while still reporting the higher version. The anchors are always kept above the version released on the base branch and above every build number already handed out, so no site can skip a step because its recorded version already covers that number. All steps are guarded (table and field existence checks, idempotent migrations), so sites that already ran them pass through again. (Wunderbyte-GmbH/moodle-mod_booking#1506)
